...
public class ExercicioMain {
	...
	public static void main(String[] args) {
		...
		int qt;
		ExercicioArraylist ap;
		Random rd = new Random();

		// lista contendo todas as apostas simuladas:
		ArrayList<ExercicioArraylist> apostas = new ArrayList<>(qtApostas);
		for (int i = 0; i < qtApostas; i++) {
			qt = 6 + rd.nextInt(10);
			ap = new ExercicioArraylist("Aposta " + i + "/" + qt, qt);
			apostas.add(ap);
		}

		// Sorteia e exibe o resultado oficial do concurso:
		System.out.println("Números sorteados: " + Concurso.numerosSorteados());
		System.out.println();

		int[] resumo = new int[3]; // Totais por quantidade de acertos

		// apuração dos resultados:
		for (ExercicioArraylist a : apostas) {
